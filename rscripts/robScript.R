...
#Price per Segment in facet grid

# 
# plotPriceInEURperMWh_Segment_A_ = NULL
# for(i in 0:length(bigDF$tick)) {
# plotPriceInEURperMWh_Segment_A_[i] = 
#   ggplot(data=bigDF, aes(x=tick, y=bigDF$PriceInEURperMWh_Segment_Country_A_[i], group=runNumber)) + 
#   geom_line() + #(aes(colour = runNumber)) +
#   xlab("Tick") +  
#   ylab("Price (EUR/MWh)") + 
#   ggtitle("Segment Price") #give the plot a title
# plot(plotPriceInEURperMWh_Segment_A_[i])
# 
# }
